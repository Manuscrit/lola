...
def train(env, *, num_episodes, trace_length, batch_size,
          ...
          mem_efficient=True, summary_len=20, **kwargs):
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            (updateModel_1, updateModel_2, update1, update2,
             player_1_value, player_2_value, player_1_target, player_2_target,
             player_1_loss, player_2_loss) = sess.run(
                [
                    mainPN[0].updateModel,
                    mainPN[1].updateModel,
                    mainPN[0].delta,
            ...
            if len(rList) % summary_len == 0 and len(rList) != 0 and updated:
                ...
                to_plot = {}
                for ii in range(len(rlog)):
                    to_plot[f'rAll[{ii}]'] = rlog[ii]

                action_log = np.sum(aList[-summary_len:], 0)
                actions_freq = {f"player_red_act_{i}": action_log[i]  for i in range(0, 4, 1)}
                actions_freq.update({f"player_blue_act_{i - 4}": action_log[i]  for i in range(4, 8, 1)})
                ...
                training_info = {
                    ...
                    "player_1_value": sum(player_1_value),
                    "player_2_value": sum(player_2_value),
                ...
                tune.report(**to_plot, **last_info, **actions_freq, **training_info)
