...
def corrections_func(mainPN, batch_size, trace_length,
    ...
    with tf.control_dependencies([print_op_15, print_op_16, print_op_17, print_op_18, print_op_19, print_op_20,
        ...
        print_op_100 = tf.print("mainPN[1].target.shape", tf.shape(mainPN[1].target))
        print_op_101 = tf.print("mainPN[1].target.mean",  tf.math.reduce_mean(mainPN[1].target, axis=0))
        print_op_102 = tf.print("mainPN[1].target", mainPN[1].target)
        print_op_103 = tf.print("mainPN[1].target - mean", (mainPN[1].target-
                                                            tf.math.reduce_mean(mainPN[1].target, axis=0)))
        with tf.control_dependencies([print_op_100, print_op_101, print_op_102, print_op_103]):
