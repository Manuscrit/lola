...
class LOLAPGMatrice(tune.Trainable):
    ...
    def _init_lola(self, env, seed, num_episodes, trace_length, batch_size,
                   ...
                   mem_efficient=True,
                   # changed_config=False, ac_lr=1.0, asymmetry=False, warmup=False,
                   summary_len=20,
                   # use_MAE=False,
                   use_toolbox_env=False,
                   clip_lola_update_norm=False,
                   # clip_loss_norm=False, opp_model, grid_size
                   # entropy_coeff=0.0, weigth_decay=0.0,  , bs_mul
        ...
        self.summaryLength = summary_len  # Number of epidoes to periodically save for analysis
        ...
        with graph.as_default() as g:
            ...
            for agent in range(self.total_n_agents):
                self.mainQN.append(Qnetwork('main' + str(agent), agent, self.env, lr=lr, gamma=gamma, batch_size=batch_size,
                                       trace_length=self.trace_length, hidden=hidden, simple_net=simple_net, sess=self.sess))
            ...
            corrections_func(self.mainQN,
                             batch_size=batch_size,
                             trace_length=self.trace_length,
                             corrections=corrections,
                             cube=self.cube,
                             # clip_lola_update_norm=clip_lola_update_norm,
                             # lola_correction_multiplier=lola_correction_multiplier,
            ...
            self.feed_dict_log_pi = {self.mainQN[0].scalarInput: array,
    ...
    def step(self):
            ...
            while j < self.max_epLength:
                ...
                if not self.use_toolbox_env:
                    ...
                    if 'player_row' in info.keys():
                        last_info.update({f"player_row_{k}": v for k, v in info['player_row'].items()})
                    if 'player_col' in info.keys():
                        last_info.update({f"player_col_{k}": v for k, v in info['player_col'].items()})
        ...
        if (self.episodes_run[agent] % self.batch_size == 0 and
                self.episodes_run[agent] > 0):

            # Sample from data buffer
            trainBatch0 = self.buffers[0].sample(self.batch_size, self.trace_length)  # Get a random batch of experiences.
            trainBatch1 = self.buffers[1].sample(self.batch_size, self.trace_length)

            sample_return0 = np.reshape(get_monte_carlo(trainBatch0[:, 2], self.y, self.trace_length, self.batch_size),
                                        [self.batch_size, -1])
            sample_return1 = np.reshape(get_monte_carlo(trainBatch1[:, 2], self.y, self.trace_length, self.batch_size),
                                        [self.batch_size, -1])

            sample_reward0 = np.reshape(trainBatch0[:, 2] - np.mean(trainBatch0[:, 2]), [-1, self.trace_length]) * self.discount
            sample_reward1 = np.reshape(trainBatch1[:, 2] - np.mean(trainBatch1[:, 2]), [-1, self.trace_length]) * self.discount

            last_state = np.reshape(np.vstack(trainBatch0[:, 3]), [-1, self.trace_length, self.env.NUM_STATES])[:,
                         -1, :]
            ...
            fetches = [
                ...
                self.mainQN[0].delta, self.mainQN[1].delta,
                self.mainQN[0].grad,
                self.mainQN[1].grad,
            ...
            if self.episodes_run[agent] % self.batch_size == 0 and self.episodes_run[agent] > 0:
                values, _, _, update1, update2, grad_1, grad_2, v0_grad_01, v1_grad_10 = self.sess.run(fetches,
                                                                                                  feed_dict=feed_dict)
        ...
        if len(self.rList) % self.summaryLength == 0 and len(self.rList) != 0 and self.updated == True:
            self.updated = False
            gamma_discount = 1 / (1 - self.gamma)
            print(self.total_steps, 'reward', np.mean(self.rList[-self.summaryLength:], 0) / gamma_discount, 'action',
                  (np.mean(self.aList[-self.summaryLength:], 0) * 2.0 / np.sum(np.mean(self.aList[-self.summaryLength:], 0))) * 100 // 1)

            action_prob = np.mean(self.aList[-self.summaryLength:], 0) * 2.0 / np.sum(np.mean(self.aList[-self.summaryLength:], 0))
            log_items['reward_agent0'] = np.mean(self.rList[-self.summaryLength:], 0)[0]
            log_items['reward_agent1'] = np.mean(self.rList[-self.summaryLength:], 0)[1]
            ...
            if self.simple_net:
                ...
                log_pi0, log_pi1 = self.sess.run([self.mainQN[0].log_pi, self.mainQN[1].log_pi],
    ...
    def compute_actions(self, policy_id:str, obs_batch:list):
        ...
        for single_obs in obs_batch:
            ...
            #         self.mainQN[agent_to_use].lstm_state_output
            #     ],
            #     feed_dict={
            #         self.mainQN[agent_to_use].state_input: obs,
            #         self.mainQN[agent_to_use].lstm_state: self.lstm_state[agent_to_use]
            #     }
            # )
            a = self.sess.run(
                [self.mainQN[agent_to_use].predict],
                feed_dict={
                    self.mainQN[agent_to_use].scalarInput: [obs]
                }
            )
